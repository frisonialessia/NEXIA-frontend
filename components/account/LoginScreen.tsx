...
import { ROL_NOMBRE, col } from "@/lib/constants";
...
export function LoginScreen() {
  ...
  function entrar(e?: React.FormEvent) {
    e?.preventDefault();
    if (!email.trim()) {
      toast.error("Escribe tu correo para entrar");
      return;
    }
    iniciarSesion(email);
  }
      ...
      <div className="w-full max-w-sm">
        ...
        >
          <label className="block">
            <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.14em] text-neutral-400">Correo</span>
            <input
              type="email"
              autoComplete="username"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="tucorreo@planta.com"
              className={input}
            />
          </label>

          <label className="mt-4 block">
            <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.14em] text-neutral-400">Contraseña</span>
            <input
              type="password"
          ...
          <p className="mt-3 text-center text-xs text-neutral-400">
            Modo demostración · el acceso no valida contraseña
          </p>
        </form>
        ...
        <div className="mt-7">
          <div className="mb-3 flex items-center gap-3">
            ...
            <span className="text-[11px] uppercase tracking-[0.14em] text-neutral-400">Cuentas de demostración</span>
            <div className="h-px flex-1 bg-neutral-200 dark:bg-neutral-800" />
          </div>
            ...
            {USUARIOS.map((u) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block truncate text-xs text-neutral-400">{ROL_NOMBRE[u.rolKey]}</span>
                </span>
