...
// ──────────────────────────────────────────────────────────────────────────
// VISTA 7 · AJUSTES
// Perfil, SISTEMA DE UNIDADES (métrico SI / imperial EE.UU.) que cambia 6
// magnitudes a la vez y afecta a los velocímetros del detalle, idioma, tema y
// umbral de sensibilidad. Portado de la vista settings + setSistema()/renderUnidades().
// ──────────────────────────────────────────────────────────────────────────
...
export function Settings() {
  const { sistema, setSistema } = useSession();
    ...
    <main className="fade-in px-6 py-8 sm:px-8">
      <div className="mx-auto max-w-3xl">
        <header className="mb-6">
          ...
          <h1 className="mt-2 font-serif text-3xl tracking-tight">Ajustes</h1>
        </header>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-neutral-400">Idioma de la interfaz</div>
                </div>
                <select className="rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-1.5 text-sm dark:border-neutral-700 dark:bg-neutral-800">
          ...
          <Seccion titulo="Umbrales de alerta">
            <div className="space-y-4">
              ...
              <p className="text-xs text-neutral-400">
                Más bajo = más sensible (más alertas). Más alto = solo casos claros.
              </p>
            </div>
          </Seccion>
          ...
          <div className="flex justify-end">
            <button
              onClick={() => toast("Ajustes guardados")}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-800"
      />
    </label>
  );
}
...
function BotonSistema({
  ...
  valor: SistemaUnidades;
  actual: SistemaUnidades;
  onClick: (s: SistemaUnidades) => void;
  brand: string;
  label: string;
}) {
  ...
  return (
    <button
      ...
      style={activo ? { background: brand, color: "#fff" } : { color: "#737373" }}
    >
      {label}
    </button>
  );
